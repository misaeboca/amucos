...
class Amuco_visits extends Admin	
{
	...
	*/
	public function add_save()
	{
		...
		$this->form_validation->set_rules('type_visit', 'Type Visit', 'trim|required');
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'subject' => ucfirst(strtolower($this->input->post('subject'))),
				'notes' => ucfirst(strtolower($this->input->post('notes'))),
			];
	...
	*/
	...
	public function get_contacts_by_id(){
		//var_dump($this->input->get('type'));exit;
		if($this->input->get('id') != null && $this->input->get('type') != null){
			$id = $this->input->get('id');
			
			if($this->input->get('type') == 'supplier'){
				$this->data['contacts'] = $this->model_amuco_contacts->get_contacts($id, 'supplier','supplier_id');
			}else{
				$this->data['contacts'] = $this->model_amuco_contacts->get_contacts($id, 'Customer','customer_id');
			}
			$this->data['success'] = true;	
